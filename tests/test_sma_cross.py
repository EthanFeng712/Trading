...
def generate_signal_stream(strategy: SmaCrossStrategy, bars: list[Bar]) -> float | None:
    ...
    for index, bar in enumerate(bars):
        strategy.generate_signal(index, previous_bar)
        previous_bar = bar
...
class SmaCrossStrategyTests(unittest.TestCase):
    def test_short_history_returns_none_without_error(self) -> None:
        strategy = SmaCrossStrategy(SmaCrossConfig(fast_window=2, slow_window=3))
        bars = [
    ...
    def test_target_size_controls_buy_and_sell_signals(self) -> None:
        config = SmaCrossConfig(fast_window=2, slow_window=3, target_size=0.4)

        buy = generate_signal_stream(
            SmaCrossStrategy(config), make_bars([3, 2, 1, 2, 3])
        )
        sell = generate_signal_stream(
            SmaCrossStrategy(config), make_bars([1, 2, 3, 2, 1])
        )

        self.assertEqual(buy, 0.4)
        self.assertEqual(sell, -0.4)
        ...
        for values in invalid_configs:
            with self.subTest(values=values):
                with self.assertRaises(ValueError):
                    SmaCrossConfig(**values)

    def test_config_rejects_non_finite_target_size(self) -> None:
        for value in (float("nan"), float("inf"), float("-inf")):
            with self.subTest(value=value):
                with self.assertRaises(ValueError):
                    SmaCrossConfig(target_size=value)

    def test_config_is_immutable(self) -> None:
        config = SmaCrossConfig()

        with self.assertRaises(FrozenInstanceError):
            setattr(config, "target_size", 0.4)
